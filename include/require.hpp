  ...
  S.register_builtin("require", [](State &S, Ptr args) -> std::shared_ptr<vdlisp::Value> {
    ...
    if (!name.empty() && name[0] != '/') {
      if (S.current_expr && S.get_source_loc(S.current_expr, loc) && !loc.file.empty()) {
        ...
        if (pos != std::string::npos) candidates.push_back(p.substr(0, pos + 1) + name);
      }
    ...
    for (const auto &cand : candidates) {
      ...
      if (std::filesystem::exists(fp, ec)) {
        ...
        else key = std::filesystem::absolute(fp, ec).string();
      }
      // if module already loaded under canonical key, return it
      auto it = S.loaded_modules.find(key);
      if (it != S.loaded_modules.end()) return it->second;
      ...
      S.loaded_modules[key] = Ptr();
      std::ostringstream ss; ss << f.rdbuf();
      Ptr e = S.parse_all(ss.str(), key);
      Ptr r;
      if (e) r = S.do_list(e, S.global);
