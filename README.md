# vd：一个迷你 Lisp 解释器（C++20）

本项目实现了一个小型 Lisp 解释器，提供交互式 REPL（readline 历史记录）、脚本执行、`require` 模块加载，以及带文件/行/列与调用链（宏展开/函数调用）的错误定位。
...
> 注意：项目在 CMake 中已设置为使用 C++20，并在 target 级别强制 `-std=c++20`。
...
- 长时间运行的压力用例：[tests/longrun.lisp](tests/longrun.lisp)
...
- `(type v)`：返回类型名 symbol，如 `number`/`string`/`function` 等
